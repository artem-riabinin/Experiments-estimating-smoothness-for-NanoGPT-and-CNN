...
wandb_run_name = 'nanogpt_unScion_estimate_blockwise_smoothness_l2_norm'
    ...
    def step(self, step):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']: 
                ...
                if master_process and (step > 0 and args.val_loss_every > 0 and step % args.val_loss_every == 0):
                    norm_params_diff = torch.norm(p.data - self.params_vector[self.iter_k])
                    norm_grad_diff = torch.norm(p.grad - self.grads_vector[self.iter_k]) 
                    norm_grad = torch.norm(p.grad)
